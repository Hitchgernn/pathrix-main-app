...
no progress, and is `aria-hidden`. Removing it would cost the screen personality
and no information — which is the test for whether a piece of motion belongs
here. Removing the words would leave the user with a dancing pastry and no idea
what the machine is doing, which is the failure the rule exists to prevent.
...
stands the buildings up. Both halves are the feature: tilting over a flat
basemap is not a view, it is a skewed map.

- **It also zooms.** MAPID carries no building heights below zoom 15, and the
  app opens at 12. Turning 3D on from the city view eases to ~15.5 as well as
  tilting, because a control that appears to do nothing is worse than one that
  moves the view.
- **There is no terrain and the UI must not imply one.** No MAPID style exposes
  a DEM (`ARCHITECTURE.md` §6.1), so there is no landscape to drape and no sky
  layer — a horizon over flat ground is a promise the data cannot keep.
- **Buildings are a neutral step, not MAPID's.** MAPID's own extrusions ramp
  `#ccc → royalblue → lightblue` by height, and they exist in `street-v2.0`
  alone. They are hidden in favour of one layer drawn in both treatments from
  `building3d` in `lib/tokens.ts` — neutral, because a saturated building
  competes with the route line on the one screen where the route line is the
  product, and because 3D must not look like a different feature per theme.
- **Buildings sit over the roads and under the type**, which is not the same as
  "below the first label": dark-v2.0 puts a water label twelve layers *beneath*
  its road network, so the anchor is the last road layer, not the first symbol.
- **Rotation belongs to 3D.** Drag-rotate and touch-pitch are off in 2D and on
  in 3D, so the control is the only way to a tilted camera and its state can
  never disagree with the screen. Leaving 3D restores north-up, not just flat.
- **It does not survive a reload.** 3D is the heavier render mode and the load
  budget (`ARCHITECTURE.md` §14) has no room for it before first paint.

### Mascot (new)
Pixel-art Bakpia, on two sheets, both regenerated by
`frontend/tools/mascot/extract.py` from the delivered contact sheets and both
played by one component (`components/Sprite.tsx`) as a CSS `steps()` walk across
a background image.

- **`bakpia-mascot.png`** — 29×32, five four-frame clips. Beside the agent's
  streaming status, at 2× (58×64).
- **`bakpia-earth.png`** — 34×46, four four-frame clips, the mascot on top of a
  globe. The location-permission screen, at 4× (136×184), playing `wave`.

Rules:

- **Integer scale only**, with `image-rendering: pixelated`. A fractional scale
  resamples the art into mush, and there is no half-step, so a size is chosen
  off that ladder rather than from a spec.
- **Its palette is illustration, not chrome.** The §Don't rule against
  introducing a UI hue governs the interface — surfaces, controls, type. A piece
  of drawn art is not a control, and neither the browns nor the Earth's blues
  and greens are new tokens; they never leak into a border, a fill, or a label.
  The Earth's blue in particular is *not* available to the chrome, which stays
  ink and one gold.
- **Dark is not a redraw.** On the mascot sheet the `#3F2412` outline loses
  contrast on a dark surface and the `#CE8F40` body carries the silhouette
  instead; the Earth sheet reads outright, its blue having plenty of contrast on
  `surface`. A light halo and a hard rim were both tried on the mascot and both
  turn it into a sticker. If a future placement genuinely fails, put a surface
  behind the sprite — do not repaint the art per theme, because then the two
  themes stop being a token swap.
- **Random where repetition would show, fixed where it would not.** The thinking
  mascot picks a clip per turn, because the same four frames on every question
  goes dead. The permission screen is shown once and picks `wave` deliberately.
  Under `prefers-reduced-motion` the thinking pick is forced to `idle` and every
  sprite holds on its first frame: still present, no longer moving.
- **A clip is only usable if every frame reads on its own**, since that is what
  a reduced-motion viewer and a mid-animation screenshot both get. `walk` runs
  off the edge of its frame and `jump` leaves the mascot cropped in mid-air;
  neither is used anywhere a sprite is shown large.
...
- **Don't** add a spinner, skeleton shimmer, or animated loading icon. State
  absence and pending status in words. The mascot is the single sanctioned
  animation in a pending state, and only alongside those words — never instead
  of them, and never as the thing that reports progress.
