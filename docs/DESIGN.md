...
### Mascot (new)
A 29×32 pixel-art Bakpia, five four-frame clips on one 116×160 sheet
(`frontend/src/assets/bakpia-mascot.png`, regenerated by
`frontend/tools/mascot/extract.py` from the delivered contact sheet). It appears
in exactly one place: next to the agent's streaming status.

- **Integer scale only**, with `image-rendering: pixelated`. Currently 2×
  (58×64). A fractional scale resamples the art into mush; there is no half-step
  between 2× and 3×, so the size is chosen from that ladder, not from a spec.
- **Its palette is illustration, not chrome.** The §Don't rule against
  ...
  of drawn art is not a control, and its nine browns are not a new token; they
  never leak into a border, a fill, or a label.
- **Dark is not a redraw.** The `#3F2412` outline loses contrast on a dark
  surface and the `#CE8F40` body carries the silhouette instead. If a future
  placement genuinely fails, put a surface behind the sprite — do not repaint
  the art per theme, because then the two themes stop being a token swap.
- **A clip is picked at random per turn**, so the moment has some life across
  repeat questions. Under `prefers-reduced-motion` the pick is forced to `idle`
  and the sprite holds on its rest frame: still present, no longer moving.
