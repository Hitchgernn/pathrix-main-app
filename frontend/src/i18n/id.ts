...
export const id = {
  ...
  "layer.transit.meta": "Halte hasil survei MAPID, Kota Yogyakarta",
  ...
  "place.communitySurvey": "Survei komunitas",
  "place.surveyNote": "Catatan survei",
  "place.surveyedBy": "Disurvei oleh",
  "place.community": "Komunitas",
  "place.surveyedAt": "Tanggal survei",
  "place.photos": (n: number) => `${n} foto survei`,
  ...
  "transit.effectiveUntil": (date: string) => `sampai ${date}`,
  "transit.freshness": (status: string) =>
    `Status data: ${status === "unverified" ? "belum diverifikasi" : status === "verified" ? "terverifikasi" : status === "stale" ? "kedaluwarsa" : status}`,
  "transit.noSchedule": "Jadwal rinci belum tersedia untuk halte ini.",
  "transit.loadingSchedule": "Memuat jadwal halte…",
  "transit.scheduleError": "Jadwal halte gagal dimuat.",
  "transit.headwayRange": (min: number, max: number) => `Setiap ${min}–${max} mnt`,
  "transit.serviceHours": (start: string, end: string) => `${start}–${end}`,
  "transit.approximate": "perkiraan",
  "transit.scheduleBasis": (basis: string) => `Dasar jadwal: ${basis}`,
  "transit.source": "Sumber jadwal",
